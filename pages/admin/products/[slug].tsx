...
const ProductAdminPage: FC<Props> = ({ product }) => {
  ...
  const onSelectedFiles = async ({ target }: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      dispatch(
        setSnackbarAlert({
          message: 'Oops, something went wrong uploading your file',
          type: 'error',
        })
      );
    }
  ...
  const onSubmit = async (formData: formData) => {
    ...
    try {
      ...
      if (!formData._id) {
        dispatch(
          setSnackbarAlert({ message: 'Product created', type: 'success' })
        );
  ...
  return (
    <AdminLayout
      title={'Product'}
      subTitle={
        product._id ? `Editing: ${product.title}` : 'Create new product'
      }
